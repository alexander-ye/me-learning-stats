...
ggplot(data=sfo_feb_flights, mapping=aes(arr_delay)) + geom_histogram(bins=50)
```
...
sfo_feb_flights %>% 
  group_by(carrier) %>% 
  summarize(
    mean_arrival_delay=mean(arr_delay), 
    sd_arrival_delay = sd(arr_delay),
    n=n(),
...
nycflights %>% select(dep_type, arr_type) %>% group_by(dep_type) %>% summarise(on_time=sum(arr_type == "on time") / n())
