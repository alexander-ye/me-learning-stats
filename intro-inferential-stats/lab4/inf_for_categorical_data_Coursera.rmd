...
```{r perc-atheist-us12}
us12 %>% group_by(response) %>% summarise(group_count=n(), proportion = n() / nrow(.))
...
- Conditions are the same as for the Central Limit Theorem:

  - Sampled observations must be independent (e.g. random sampling, n \< 0.1 \* N)

  - Either the population distribution is normal, or the sample size is sufficiently large

If the conditions for inference are reasonable, we can either calculate the standard error and construct the interval by hand, or allow the `inference` function to do it for us.

```{r us-atheism-ci}
ci <- inference(y = response, data = us12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
```
...
spain_atheism <- atheism %>% 
  ...
  group_by(year) %>% 
  select(c("year", "response"))


ht_spain <- inference(y=response, x=as.factor(year), data=spain_atheism, type="ht", statistic="proportion", method="theoretical", success="atheist", alternative="twosided", null=0)
```
...
ht_us <- inference(y=response, x=as.factor(year), data=us_atheism, type="ht", statistic="proportion", method="theoretical", success="atheist", alternative="twosided", null=0)
```
...
```{r sample-size}
p_conservative <- 0.5
n <- (1.96^2 * p_conservative * (1 - p_conservative)) / 0.01^2
n
