...
z_star_95_aye_method <- abs(qnorm((1 - 0.95) / 2))
z_star_95 == z_star_95_aye_method
z_star_95
```
...
```{r ci}
calculate_ci <- function(x, se, c=0.95) {
...
samp %>%
  summarise(lower = mean(area) - z_star_95 * sd(area) / sqrt(n),
            ...
            lower_test = calculate_ci(mean(area), sd(area) / sqrt(n))[1],
            upper_test = calculate_ci(mean(area), sd(area) / sqrt(n))[2]
  )
...
3.  What does "95% confidence" mean?
    ...
    <li>95% of the time the true average area of houses in Ames, Iowa,
    will be in this interval.</li>

    <li>95% of random samples of size 60 will yield confidence intervals
    that contain the true average area of houses in Ames, Iowa.</li>
...
ci_99 %>% summarize(
  ci_capture_pop_mean = sum(capture_mu == "yes") / n()
)
